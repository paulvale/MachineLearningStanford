...
m = size(X, 1);
[ Theta1M Theta1N ] = size(Theta1);
[ Theta2M Theta2N ] = size(Theta2);
...
%         computed in ex4.m
X = [ones(m,1) X];
% Calculate of features in the hidden layer
a2 = sigmoid(X*Theta1');
...
for(c = 1:num_labels),
  % Set initial yk
  yk = (y==c);
  % Set the correct value of h
  h = a3(:,c);
  J = J + ((yk'*log(h)) + (1-yk)'*log(1-h))
end;

Theta1R = Theta1(:,2:Theta1N);
Theta2R = Theta2(:,2:Theta2N);

reg1 = sum(sum(Theta1R.^2,2));
reg2 = sum(sum(Theta2R.^2,2));
J = -1/m * J + (lambda/(2*m))*(reg1 + reg2);
...
for(c = 1:m),
  ...
  % Step2 : Error initialization
  value = (1:num_labels);
  y_k = (value==y(c));
  delta3 = a_3 - y_k';

  % Step3 : Hidden layer error
